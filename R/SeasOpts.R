"setSeasOpts" <-
  function(){
    ...
    options(seas.main=list(fmt="%s\n%s",rngsep="-",show.id=TRUE,show.fun=TRUE),
            ...
            seas.temp=list(col=c("lightgrey","red"),lwd=2),
            seas.precip=list(col="grey",   density=NULL,angle =  45),
            seas.rain=list(col="lightblue",density=NULL,angle =  45),
            seas.snow=list(col="lightgrey",density=NULL,angle = -45),
